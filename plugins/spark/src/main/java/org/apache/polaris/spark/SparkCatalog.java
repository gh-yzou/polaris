...
public class SparkCatalog implements TableCatalog, SupportsNamespaces {
  ...
  public void initialize(String name, CaseInsensitiveStringMap options) {
    ...
    if (this.sparkTableCatalog instanceof CatalogExtension) {
      ...
      ((CatalogExtension) this.sparkTableCatalog).setDelegateCatalog(this.genericTableSparkCatalog);
    }
  ...
  public Table createTable(
    ...
    try {
      ...
      // Dataset<Row> df = spark.createDataFrame(emptyRows, schema);
      // String location = properties.get(TableCatalog.PROP_LOCATION);
      // df.write().mode("overwrite").format(provider).save(location);
  ...
  public Identifier[] listTables(String[] namespace) {
    return genericTableSparkCatalog.listTables(namespace);
  }
